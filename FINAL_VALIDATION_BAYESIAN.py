#!/usr/bin/env python3
"""
FINAL VALIDATION WITH BAYESIAN RIDGE: Measure actual accuracy improvement
Compares OLD algorithm (Ridge) vs NEW algorithm (Bayesian Ridge multi-level)
"""
...
def cross_validate_regression(X, y, use_bayesian=False):
    """Leave-One-Out Cross-Validation with Ridge or Bayesian Ridge"""
    if len(X) < 3:
        return None
...
def new_algorithm(df):
    """
    NEW algorithm: Bayesian Ridge with hybrid multi-level and validation R²
    This should correctly handle n=3 groups with automatic regularization
    """
    print("\n🟢 Running NEW Algorithm (Bayesian Ridge)...")
    print("   - Uses Bayesian Ridge with automatic regularization")
    ...
    for prefix in sorted(df['prefix'].unique()):
        ...
        if n_anchors >= 10:
            ...
            if len(np.unique(X)) >= 2:
                ...
                validation_r2 = cross_validate_regression(X, y, use_bayesian=True)
