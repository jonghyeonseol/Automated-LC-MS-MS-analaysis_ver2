...
class GangliosideDataProcessor:
    ...
    def process_data(
        ...
        print(
            f"   - 이성질체 후보: {sum(1 for info in rule23_results['sugar_analysis'].values() if info['can_have_isomers'])}"
        )
        ...
        for suffix in df["suffix"].unique():
            ...
            for group in rt_groups:
                if len(group) > 1:
                    ...
                    sugar_counts.sort(
                        key=lambda x: (-x[1], x[0]["Log P"])
                    )  # 당 개수 내림차순, Log P 오름차순
