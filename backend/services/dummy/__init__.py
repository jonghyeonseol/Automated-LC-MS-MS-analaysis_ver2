...
try:
    from .processors import DummyGangliosideDataProcessor, DummyVisualizationService
    ...
    class DummyGangliosideDataProcessor:
        ...
        def process_data(self, df, data_type="Porcine"):
            ...
            if df is not None and not df.empty:
                total = len(df)
                anchor_count = len(df[df['Anchor'] == 'T']) if 'Anchor' in df.columns else 0
                ...
                if 'Name' in df.columns:
                    ...
                    # GT1 계열 확인
                    gt1_compounds = df_copy[df_copy['prefix'].str.contains('GT1', na=False)]
                    if len(gt1_compounds) > 0:
                        print(f"🎯 GT1 계열 발견: {len(gt1_compounds)}개")
                        
                        # GT1(42:2;O2) 특별 확인
                        target_gt1 = gt1_compounds[gt1_compounds['Name'].str.contains('GT1\\(42:2;O2\\)', na=False)]
                        if len(target_gt1) > 0:
                            print(f"🎯 GT1(42:2;O2) 특별 발견: {len(target_gt1)}개 - 강제 유효 처리!")
                ...
                outlier_count = total - valid_count
                ...
                for i, (_, row) in enumerate(df.iterrows()):
                    ...
                    if row.get('Anchor') == 'T':
                        ...
                        row_dict['outlier_reason'] = 'Rule 1: Simulated outlier for demo'
                ...
                return {
                    ...
                    "outliers": outliers,
                    "regression_results": {
                        "GD1": {"r2": 0.95, "slope": -0.5, "intercept": 8.0, "passes_threshold": True},
                        "GM1": {"r2": 0.92, "slope": -0.4, "intercept": 8.5, "passes_threshold": True}
                    },
                    "settings_impact": {
