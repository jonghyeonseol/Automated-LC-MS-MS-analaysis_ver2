...
class DummyGangliosideDataProcessor:
    ...
    def update_settings(
        ...
        print(
            f"⚙️ 설정 업데이트: 이상치={self.outlier_threshold}, R²={self.r2_threshold}, RT={self.rt_tolerance}"
        )
    ...
    def process_data(
        ...
        print(
            f"🔬 개선된 더미 분석 실행: 이상치={self.outlier_threshold}, R²={self.r2_threshold}, RT={self.rt_tolerance}"
        )
        ...
        for prefix in df["prefix"].unique():
            ...
            if len(anchor_compounds) >= 1 or len(prefix_group) >= 2:
                ...
                if r2 >= effective_threshold:
                    ...
                    print(
                        f"  ✅ {prefix} 회귀분석 통과: R² = {r2:.3f} >= {effective_threshold:.3f}"
                    )
                    ...
                    for _, row in prefix_group.iterrows():
                        ...
                        if row["Anchor"] == "T":
                            ...
                            if abs(std_residual) >= effective_outlier_threshold:
                                row_dict[
                                    "outlier_reason"
                                ] = f"Rule 1: |Std residual| = {abs(std_residual):.2f} >= {effective_outlier_threshold:.2f}"
                                outliers.append(row_dict)
                                print(
                                    f"    ❌ {row['Name']}: 이상치 (잔차 = {abs(std_residual):.2f})"
                                )
                            else:
                                valid_compounds.append(row_dict)
                                print(
                                    f"    ✅ {row['Name']}: 유효 (잔차 = {abs(std_residual):.2f})"
                                )
                    ...
                    print(
                        f"  ⚠️ {prefix} R² 미달: {r2:.3f} < {effective_threshold:.3f}, 하지만 Anchor는 유효 처리"
                    )
        ...
        print(
            f"🎯 Anchor 보존률: {anchor_in_valid}/{anchor_compounds} ({anchor_success_rate:.1f}%)"
        )
        ...
        return {
            "statistics": {
                ...
                "rule_breakdown": {
                    ...
                    "auto_classification": len(
                        [c for c in valid_compounds if c.get("auto_classification")]
                    ),
