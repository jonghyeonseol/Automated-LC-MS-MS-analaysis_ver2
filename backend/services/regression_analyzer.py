...
class RegressionAnalyzer:
    ...
    def perform_comprehensive_regression(self, x_data: np.ndarray, y_data: np.ndarray, 
        ...
        try:
            ...
            influence_diagnostics = self._influence_diagnostics(x_data, y_data, basic_regression)
    ...
    def _perform_ols_regression(self, x_data: np.ndarray, y_data: np.ndarray) -> Dict[str, Any]:
        """기본 OLS 회귀분석 수행"""
        ...
        adjusted_r2 = 1 - (ss_residual / (n - 2)) / (ss_total / (n - 1)) if n > 2 else r2
        ...
        # F-통계량
        f_statistic = (ss_regression / 1) / (ss_residual / df) if df > 0 and ss_residual > 0 else 0
        ...
        return {
            ...
            'standardized_residuals': (residuals / np.std(residuals)).tolist() if np.std(residuals) > 0 else residuals.tolist(),
            'n_observations': n,
            'degrees_freedom': df,
            'equation': f'RT = {slope:.4f} * Log_P + {intercept:.4f}'
        }
    
    def _comprehensive_residual_analysis(self, residuals: List[float], 
    ...
    def _durbin_watson_test(self, residuals: np.ndarray) -> Dict[str, Any]:
        ...
        n = len(residuals)
        if n < 2:
            return {'statistic': 2.0, 'interpretation': '데이터 부족', 'is_problematic': False}
    ...
    def _breusch_pagan_test(self, residuals: np.ndarray, 
                           predicted_values: np.ndarray) -> Dict[str, Any]:
    ...
    def _influence_diagnostics(self, x_data: np.ndarray, y_data: np.ndarray, 
                              regression_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _advanced_outlier_detection(self, x_data: np.ndarray, y_data: np.ndarray,
        ...
        for i in range(n):
            if combined_outliers[i]:
                outlier_info = {
                    'index': int(i),
                    'name': compound_names[i] if compound_names and i < len(compound_names) else f"Point_{i}",
    ...
    def _test_regression_assumptions(self, residuals: List[float], 
        ...
        # 1. 선형성 (잔차 vs 예측값의 패턴)
        linearity_corr = np.corrcoef(predicted_values, residuals)[0, 1] if len(residuals) > 1 else 0
        ...
        assumptions['overall_assessment'] = {
            ...
            'violated_assumptions': [
                name for name, test in assumptions.items() 
    ...
    def _calculate_prediction_intervals(self, x_data: np.ndarray, y_data: np.ndarray,
                                      regression_result: Dict[str, Any]) -> Dict[str, Any]:
        """예측 구간 계산"""
    ...
    def _evaluate_model_quality(self, basic_regression: Dict[str, Any],
                               residual_diagnostics: Dict[str, Any],
                               assumption_tests: Dict[str, Any]) -> Dict[str, Any]:
        """모델 품질 종합 평가"""
        ...
        # 2. 통계적 유의성 점수
        quality_scores['statistical_significance'] = 100 if f_p_value < 0.05 else 50 if f_p_value < 0.1 else 0
    ...
    def _generate_recommendations(self, basic_regression: Dict[str, Any],
                                 residual_diagnostics: Dict[str, Any],
                                 assumption_tests: Dict[str, Any]) -> List[str]:
        """개선 권장사항 생성"""
    ...
    def _generate_analysis_summary(self, basic_regression: Dict[str, Any],
        ...
        return {
            ...
            'outliers_detected': outlier_analysis['outlier_summary']['n_total_outliers'],
            'outlier_percentage': outlier_analysis['outlier_summary']['outlier_percentage'],
            'meets_quality_threshold': model_quality['meets_threshold'],
            'key_recommendations': model_quality['recommendations'][:3]  # 상위 3개 권장사항
        }
    
    def _minimal_regression_result(self, x_data: np.ndarray, y_data: np.ndarray) -> Dict[str, Any]:
    ...
    def _empty_regression_result(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'model_quality': {'overall_score': 0.0, 'grade': 'No Data', 'reliability': 'None'},
    ...
    def _error_regression_result(self, error_message: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'model_quality': {'overall_score': 0.0, 'grade': 'Error', 'reliability': 'None'},
