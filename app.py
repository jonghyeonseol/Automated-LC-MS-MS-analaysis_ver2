...
# 분석 서비스 import
try:
    from backend.services.data_processor import GangliosideDataProcessor as GangliosideProcessor
    ...
    print("더미 모듈을 사용합니다...")
    from backend.services.dummy import DummyGangliosideDataProcessor as GangliosideProcessor
    from backend.services.dummy import DummyVisualizationService as VisualizationService
...
def analyze_data():
    ...
    try:
        ...
        print(f"⚙️ 설정: outlier={outlier_threshold}, r2={r2_threshold}, rt={rt_tolerance}")
        ...
        print(f"✅ 분석 완료: {results['statistics']['success_rate']:.1f}% 성공률")
...
def create_visualizations():
    ...
    try:
        ...
        return jsonify({"error": f"시각화 생성 중 오류 발생: {str(e)}"}), 500
    ...
    try:
        if request.method == 'GET':
            ...
            if not (1.0 <= outlier_threshold <= 5.0):
                return jsonify({"error": "outlier_threshold는 1.0-5.0 범위여야 합니다."}), 400
            if not (0.90 <= r2_threshold <= 0.999):
                return jsonify({"error": "r2_threshold는 0.90-0.999 범위여야 합니다."}), 400
            if not (0.05 <= rt_tolerance <= 0.5):
                return jsonify({"error": "rt_tolerance는 0.05-0.5 범위여야 합니다."}), 400
        ...
        return jsonify({"error": f"설정 처리 중 오류 발생: {str(e)}"}), 500
...
def sample_test():
    ...
    try:
        ...
        if test_type == 'basic':
            ...
            return jsonify({"error": "지원하지 않는 테스트 타입입니다."}), 400
        ...
        return jsonify({"error": f"샘플 테스트 중 오류 발생: {str(e)}"}), 500


def _generate_results_csv(results):
    """분석 결과를 CSV 형태로 변환"""
    lines = ['Name,RT,Volume,Log P,Anchor,Status,Classification,Predicted_RT,Residual,Std_Residual,Outlier_Reason\n']
    ...
    for compound in results.get('valid_compounds', []):
        ...
        lines.append(f'"{compound["Name"]}",{compound["RT"]},{compound["Volume"]},{compound["Log P"]},{compound["Anchor"]},Valid,"True Positive",{predicted_rt},{residual},{std_residual},""\n')
    ...
    for outlier in results.get('outliers', []):
        ...
        lines.append(f'"{outlier["Name"]}",{outlier["RT"]},{outlier["Volume"]},{outlier["Log P"]},{outlier["Anchor"]},Outlier,"False Positive",{predicted_rt},{residual},{std_residual},"{reason}"\n')
...
def _get_complex_sample_data():
    ...
    return {
        'Name': [
            'GD1a(36:1;O2)', 'GD1b(36:1;O2)', 'GD1a+OAc(36:1;O2)', 'GM1a(34:1;O2)', 
            'GM1a+2OAc(34:1;O2)', 'GM3(36:1;O2)', 'GD3(36:1;O2)', 'GT1b(36:1;O2)', 
            'GQ1c(36:1;O2)', 'GP1(36:1;O2)', 'GD1+dHex(36:1;O2)', 'GD1+HexNAc(36:1;O2)'
        ],
        'RT': [9.572, 9.823, 10.125, 10.452, 11.234, 10.606, 10.126, 8.701, 8.234, 7.851, 9.823, 9.572],
        'Volume': [1000000, 850000, 650000, 500000, 320000, 2000000, 800000, 1200000, 580000, 300000, 750000, 690000],
        'Log P': [1.53, 1.48, 2.15, 4.00, 4.85, 7.74, 5.27, -0.94, -3.36, -5.88, 1.48, 1.53],
...
def _get_challenge_sample_data():
    ...
    return {
        'Name': [
            'GD1a(36:1;O2)', 'GM1a(36:1;O2)', 'GM3(36:1;O2)', 'GD3(36:1;O2)', 'GT1b(36:1;O2)',
            'Unknown_1(36:1;O2)', 'Contamination(36:1;O2)', 'Noise_signal(unknown)', 'Fragment_ion(36:1;O2)', 'Artifact(36:1;O2)'
        ],
        'RT': [9.572, 10.852, 10.306, 11.726, 8.201, 12.543, 15.234, 8.765, 10.456, 14.123],
        'Volume': [1000000, 500000, 2000000, 800000, 1200000, 450000, 150000, 50000, 2500000, 80000],
        'Log P': [1.53, 4.20, 7.94, 5.47, -0.74, 8.25, 12.45, -2.15, 7.82, 9.87],
