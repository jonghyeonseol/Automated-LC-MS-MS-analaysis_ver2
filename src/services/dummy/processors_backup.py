...
class DummyGangliosideDataProcessor:
    ...
    def update_settings(
        ...
        print(
            f"⚙️ 더미 설정 업데이트: outlier={self.outlier_threshold}, r2={self.r2_threshold}, rt={self.rt_tolerance}"
        )
    ...
    def process_data(
        ...
        print(
            f"🔬 더미 분석 실행: threshold={self.outlier_threshold}, r2={self.r2_threshold}, rt={self.rt_tolerance}"
        )
        ...
        for prefix in df["prefix"].unique():
            ...
            if len(anchor_compounds) >= 1 or len(prefix_group) >= 2:  # 최소 요구사항 완화
                print(
                    f"📊 {prefix} 그룹: {len(anchor_compounds)}개 Anchor, {len(prefix_group)}개 총 화합물"
                )
                ...
                if r2 >= effective_threshold:
                    ...
                    for _, row in prefix_group.iterrows():
                        ...
                        if row["Anchor"] == "T":
                            ...
                            if abs(std_residual) >= effective_outlier_threshold:
                                row_dict[
                                    "outlier_reason"
                                ] = f"Rule 1: |Std residual| = {abs(std_residual):.2f} >= {effective_outlier_threshold}"
                    ...
                    print(
                        f"  ⚠️ {prefix} 그룹: R² = {r2:.3f} < {effective_threshold:.3f}, 하지만 Anchor는 유효 처리"
                    )
                    ...
                    for _, row in prefix_group.iterrows():
                        ...
                        if row["Anchor"] == "T":
                            ...
                            row_dict[
                                "outlier_reason"
                            ] = f"Rule 1: Low R² = {r2:.3f} < {effective_threshold:.3f}"
                            outliers.append(row_dict)
        ...
        success_rate = (
            (final_valid / total_compounds) * 100 if total_compounds > 0 else 0
        )

        # Anchor 통계 확인
        anchor_in_valid = len([c for c in valid_compounds if c.get("Anchor") == "T"])
        anchor_success_rate = (
            (anchor_in_valid / anchor_compounds) * 100 if anchor_compounds > 0 else 0
        )

        # 설정 영향도 계산
        setting_impact = {
            "outlier_strictness": "High"
            if self.outlier_threshold >= 3.0
            else "Medium"
            if self.outlier_threshold >= 2.0
            else "Low",
            "r2_strictness": "Very High"
            if self.r2_threshold >= 0.99
            else "High"
            if self.r2_threshold >= 0.95
            else "Medium",
            "rt_precision": "High"
            if self.rt_tolerance <= 0.1
            else "Medium"
            if self.rt_tolerance <= 0.2
            else "Low",
        ...
        print(
            f"🎯 Anchor 보존률: {anchor_in_valid}/{anchor_compounds} ({anchor_success_rate:.1f}%) - 모든 Anchor는 참값으로 처리되어야 함"
        )
